...
type Tab = 'dashboard' | 'planner' | 'tasks' | 'settings';
...
function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('familydashboard:task-added', handler as EventListener);
    const refresh = async () => {
      try { const list = await dal.getTasks(); setTasks(list); } catch (err) { console.warn('refresh tasks failed', err); }
    };
    window.addEventListener('familydashboard:data-changed', refresh as EventListener);
    return () => {
      window.removeEventListener('familydashboard:task-added', handler as EventListener);
  ...
  <button className={`tab-button ${tab === 'settings' ? 'active' : ''}`} onClick={() => setTab('settings')}>Settings</button>
      ...
      {tab === 'dashboard' && (
            ...
            <div className="w-100">
        <WeekView tasks={tasks} onEdit={handleEdit} onDelete={handleDelete} onAdd={handleAdd} mode="rows" weekStart={new Date()} daysCount={weekDays} />
